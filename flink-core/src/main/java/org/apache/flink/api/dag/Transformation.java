 ...
 *
 *  除了PhysicalTransformation  之外，还有一部分转换操作直接继承自Transformation抽象类
 *  这些算子本身就是物理转换操作，不支持链化操作，因此不会将其与其他算子放置在同一个 subTask下运行
 *
 *  SelectTransformation  根据用户提供的selectName从上游DataStream 中选择需要输出到下游的数据
 *  PartitionTransformation   支持对上游的DataStream中的数据分区，分区策略通过指定的StreamPartitioner
 *  决定，例如当用户执行DataStream.reblance()方法时  就会创建StreamPartitioner实现类 ReblancePartitioner
 *  实现类
 *  UnionTransformatition 用于对多个输入合并
 *  SideOutputTransformation  用于根据OutPutTag 刷选上游DataStream中的数据并下发
 *
 *  FeedbackTransformatition  用于迭代计算中但输入反馈数据流节点的转换操作
 *  CoFeedbackTransformation  用于迭代计算中双输入流反馈数据流节点的转换操作
 */
@Internal
