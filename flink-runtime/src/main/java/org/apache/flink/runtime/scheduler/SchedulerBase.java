...
public abstract class SchedulerBase implements SchedulerNG, CheckpointScheduling {
    ...
    /** 之前的版本  executionGraph 是jobMaster的成员 */
    ...
    //一个jobMaster 对应 一个调度器 ,一个调度器 对应 一个版本
    ...
    public void declineCheckpoint(final DeclineCheckpoint decline) {
        ...
        if (checkpointCoordinator != null) {
            ioExecutor.execute(
                    () -> {
                        try {
                            /** jobMaster端接受declineCheckpoint消息 最终会由checkpointCoordinator来处理   */
