...
/**  基于此类有两种实现：session模式  和  job模式
 *  SessionDispatcherLeaderProcess用于对多个jobGraph进行恢复与提交 ，在高可用集群下通过jobGraphStore中
 *  存储jobGraph进行存储与恢复
 *  JobDispatcherLeaderProcess 针对单个的job graph
 *
 *  DispatcherLeaderProcess 的实现类都是通过工厂创建的
 *   例如：
 *  SessionDispatcherLeaderProcessFactoryFactory 创建 SessionDispatcherLeaderProcessFactory
 *  SessionDispatcherLeaderProcessFactory 创建 SessionDispatcherLeaderProcess
 * */
@Internal
