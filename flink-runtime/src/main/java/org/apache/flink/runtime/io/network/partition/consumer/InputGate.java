...
/**
 * StreamTask通过InputGate从上游其他Task获取到数据。每个InputGate包含一个或多个InputChannel，
 * 根据数据是否走网络通信，这些InputChannel分为RemoteInputChannel和LocalInputChannel。
 * 其中RemoteInputChannel使用Netty通过网络从上游task的ResultSubPartition获取数据，
 * 适用与本task和上游task运行在不同集群节点的情况。和它相反的是LocalInputChannel，
