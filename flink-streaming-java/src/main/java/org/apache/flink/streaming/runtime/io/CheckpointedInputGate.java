...
public class CheckpointedInputGate implements PullingAsyncDataInput<BufferOrEvent>, Closeable {
    ...
    /**对于SourceTask，其checkpint的执行由jobmaster发送的checkpoint消息触发，而对于非SourceTask，
     * 则是依靠上游task的checkpointBarrier消息触发。由以上分析可知，在SourceTask进行checkpoint时，
     * 会向下游发送CheckpointBarrier消息，而下游的task正是拿到该消息后，进行checkpoint操作。
     *
     * 对于非SourceTask，其一直通过循环从上游读取消息，当接收一条消息后，会对消息类型进行判断，
     * 如果是CheckpointBarrier类型的消息则会进一步判断是需要对齐或是进行checkpoint。
