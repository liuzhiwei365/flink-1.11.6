 ...
 * 触发器确定窗口(由窗口分配程序形成)何时准备由窗口函数处理 每个WindowAssigner
 * 都带有一个默认触发器
 *
 * onElement()     添加到每个窗口的元素都会调用
 * onEventTime()     事件时间计时器 触发时，将调用
 * onProcessingTime()    处理时间计时器 触发时，将调用
 * onMerge()    与有状态触发器相关，在两个触发器对应的窗口合并时合并它们的状态
