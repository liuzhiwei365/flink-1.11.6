...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>> extends AbstractInvokable
     ...
     * checkpoint操作对状态数据进行快照操作时所用的异步线程池，避免checkpoint操作阻塞主线程
     * */
    ...
    /** 采用actor 模型的邮箱机制取代之前的多线程模型，让task的执行过程变成单线程加阻塞队列的形式*/
    ...
    protected StreamTask(
        ...
        /**  mailbox是task级别的，每个task都有一套 mailboxprocessor线程处理特殊事件和inputChannel数据  */
    ...
    public Future<Boolean> triggerCheckpointAsync(
        ...
        /** 会向mailbox 发送 mail, 并和其他mail一样，依次被mailboxProcessor 执行 */
        mainMailboxExecutor.execute(
                () -> {
    ...
    private boolean triggerCheckpoint(
        ...
        try {
             ...
             *   将checkpoint消息发送至下游task  执行checkpoint
             */
            boolean success =
                    performCheckpoint(checkpointMetaData, checkpointOptions, checkpointMetrics);
            if (!success) {
                /** 如果checkpoint 没有成功，则回退  */
    ...
    private static <OUT>
        ...
        for (int i = 0; i < outEdgesInOrder.size(); i++) {
            ...
            /** 最终，一个输出的 edge 对应 一个 record writer */
            recordWriters.add(
                    createRecordWriter(
                            edge,
                            i,
    ...
    private static <OUT> RecordWriter<SerializationDelegate<StreamRecord<OUT>>> createRecordWriter(
        ...
        LOG.debug(
                ...
                outputIndex,
                taskName);
        /** 即ResultPartition  会在存储需要下发的 buffer ，并等待下游节点向上游节点发送数据消费请求 */
        ...
        /**
         *   创建并装配 RecordWriter （设定Metrix 监控 等）
         *   RecordWriter负责将buffer 数据写入指定的ResultPartition
         *
         *   RecordWriter 主要包含RecordSerializer  和  ResultPartitionWriter 两个组件
         *   RecordSerializer 用于序列化操作, ResultPartitionWriter用于写入操作
         *
         *   build该对象的时候，会根据 isBroadcast条件判断 创建 BroadcastRecordWriter
         *   还是 ChannelSelectorRecordWriter
         * */
