...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
     ...
     *  per-job 模式的弊端：
     *
     *  main 方法都是在客户端执行的,在 main 方法开始执行直到 env.execute() 方法之前，客户端也需要做一些工作，即：
     *  1获取作业所需的依赖项；2通过执行环境分析并取得逻辑计划，即StreamGraph→JobGraph 3将依赖项和JobGraph上传到集群中。
     *
     * （   目前，对于per job模式，jar包的解析、生成JobGraph是在客户端上执行的，然后将生成的jobgraph提交到集群。
     *  很多公司都会有自己的实时计算平台，用户可以使用这些平台提交flink任务，如果任务特别多的话，那么这些生成JobGraph、
     *  提交到集群的操作都会在实时平台所在的机器上执行，那么将会给服务器造成很大的压力。
     *  此外这种模式提交任务的时候会把本地flink的所有jar包先上传到hdfs上相应 的临时目录，这个也会带来大量的网络的开销，
     *  所以如果任务特别多的情况下，平台的吞吐量将会直线下降。  ）
     *
     * application 模式是 per-job模式的改良 ：
     * 目前 Application 模式支持 Yarn 和 K8s 的部署方式，Yarn Application 模式会在客户端将运行任务需要的依赖都
     * 都提前上传到 hdfs 。（服务端自己提交 jobGraph）
     *
    ...
    private ClusterClientProvider<ApplicationId> deployInternal(
        ...
        // 检查部署配置 jar包路径、conf路径、Yarn cpu核数配置
        ...
        //分离模式，yarn client 提交任务后马上返回相关信息。 后面任务的过程，与任务无关了。
        // 只负责生孩子，不负责养孩子
        if (detached) {
    ...
    private ApplicationReport startAppMaster(
        ...
        //ApplicationSubmissionContext 是yarn的api
        ...
        /** 将用户配置的 shipFiles 和 日志配置文件 都加入到  systemShipFiles 中 */
        ...
        /** classpath assembler 类路径组装--------------------------------------------------------->*/
        StringBuilder classPathBuilder = new StringBuilder();
        if (userJarInclusion == YarnConfigOptions.UserJarInclusion.FIRST) {
            for (String userClassPath : userClassPaths) {
                //这里  的 File.pathSeparator 是 ":" 冒号 ， 类路径分割符
        ...
        classPathBuilder
                .append(localResourceDescFlinkJar.getResourceKey()).append(File.pathSeparator);
        /** <------------------------------------------------------------classpath assembler 类路径组装*/
        ...
        String keytab = configuration.getString(SecurityOptions.KERBEROS_LOGIN_KEYTAB);//"security.kerberos.login.keytab"
        if (keytab != null) {
            boolean localizeKeytab =
                    flinkConfiguration.getBoolean(YarnConfigOptions.SHIP_LOCAL_KEYTAB);//"yarn.security.kerberos.ship-local-keytab"
            localizedKeytabPath =
                    flinkConfiguration.getString(YarnConfigOptions.LOCALIZED_KEYTAB_PATH);//"yarn.security.kerberos.localized-keytab-path"
            if (localizeKeytab) {
