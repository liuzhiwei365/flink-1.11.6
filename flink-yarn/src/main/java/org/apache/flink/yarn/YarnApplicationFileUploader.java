...
class YarnApplicationFileUploader implements AutoCloseable {
    ...
    YarnLocalResourceDescriptor registerSingleLocalResource(
            final String key,  //资源名
            final Path resourcePath,
            final String relativeDstPath,
            final boolean whetherToAddToRemotePaths,  //是否加入到 remotePaths 数据结构
            final boolean whetherToAddToEnvShipResourceList)   //是否加入到 EnvShipResourceList 数据结构
            throws IOException {
        ...
        final Tuple2<Path, Long> remoteFileInfo =
                uploadLocalFileToRemote(resourcePath, relativeDstPath); //上传本地文件到远程

        final YarnLocalResourceDescriptor descriptor =
                new YarnLocalResourceDescriptor(
                        key,
                        remoteFileInfo.f0,   //dst 目标地址
                        localFile.length(),
                        remoteFileInfo.f1,  //lastModified
